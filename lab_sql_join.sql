...
select first_name, last_name, sum(amount) from payment left join customer using(customer_id)
group by customer_id
order by last_name;
...
select title, count(film_id) from rental left join inventory using(inventory_id) left join film using(film_id)
group by film_id
