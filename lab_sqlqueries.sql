...
select district_id, count(distinct client_id) from bank.client
...
select account_id, type, sum(amount) total_amount from bank.trans
...
select replace(replace(type, 'VYDAJ', 'Outgoing'),'PRIJEM','Incoming') as transaction_type, account_id, 
round(sum(amount)) total_amount from bank.trans
where account_id = 396
group by type
order by type
